...
bool fuse_loop(Loop *L0, Loop *L1, ScalarEvolution &SE,
  ...
  if (branch_latch_0 == 1) {
    if (adj_status == GuardedAdjacent) {
      ...
      BranchInst *latch_term0 =
          dyn_cast<BranchInst>(BB_latch0->getTerminator());
...
bool printCheckResult(bool condition, const std::string &message,
                      int checkNumber) {
  if (condition) {
...
bool analyze_loop(Function &F, LoopInfo &LI, DominatorTree &DT,
  ...
  while (true) {
    ...
    for (int i = Worklist.size() - 1; i >= 1; --i) {
      ...
      modified = true;
      modified_any = true;
      break;  // esci per ricostruire CFG e analisi
    }
